#' @title 计算加权进界邻体散射荫蔽度在特定局域临界值下最适的校正系数
#'
...
#' @param tan 局域临界值
#'
#' @return 加权进界邻体散射荫蔽度在特定局域临界值下最适的校正系数
#' @export
#'
#' @examples
#' b=Understory.light::b
#' Understory.light::MI.Diffuse_Tan.S(minx=50, maxx=100, miny=50, maxy=100, b, seq=5, tan=2.8)
MI.Diffuse_Tan.S=function (minx, maxx, miny, maxy, b, seq, tan)
{
  library(sp)
  library(gstat)
  library(tcltk)
  ...
  nls = nls(Percentile_Rank_of_Tan_dependence_S ~(atan(Tan_dependence_S/(MI)-pi)+pi/2)/pi , start = list(MI = 1), algorithm = "port")
  res = summary(nls)
  MI = res$parameters[1, 1]
  Revise_Tan_dependence_S = (atan(Tan_dependence_S/(MI)-pi)+pi/2)/pi
