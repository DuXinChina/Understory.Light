plot.Scale.dependence.Beam_S.Krig=function(minx, maxx, miny, maxy, b, seq, scale, mi, MI,bandwidth)
{
library(sp)
library(gstat)
library(tcltk)
library(ggplot2)
  ...
  Scale.dependence.Beam_S.mult=function(a,b,scale,mi,MI,bandwidth)
  {
    library(tcltk)
    ...
    star_time = Sys.time()
    for (j in 1:nrow(a)) {
      d[j, ] = cbind(as.matrix(a[j, 1:2]), as.matrix(Scale.dependence.Beam_S.single(a[j,],b,scale,mi,MI,bandwidth)))
...
data = Scale.dependence.Beam_S.mult(basexy,b,scale,mi,MI,bandwidth)
data1 = data
