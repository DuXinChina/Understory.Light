#' @title 绘制单一林下位点的加权进界邻体散射荫蔽度的示意图
#'
...
#'
#' @return 单一林下位点的加权进界邻体散射荫蔽度的示意图
#' @export
#'
#' @examples
#' a=data.frame(x=75,y=75,size=0)
#' b=Understory.light::b
#' Understory.light::plot.Tan.dependence.Diffuse_S.single(a=a,b=b,tan=2.8,MI=10.660,wei=0.5)
#' Understory.light::plot.Tan.dependence.Diffuse_S.single(a=a,b=b,tan=2.8,MI=10.660,wei=0.5,tan2=1)
plot.Tan.dependence.Diffuse_S.single=function (a, b, tan, MI,wei,tan2)
{
  library(ggplot2)
  if(missing(tan2))
  {tan2=NA}
  Tan.dependence.Diffuse_S.single = function(a, b, tan, MI,wei,tan2)
  {
    if(missing(tan2))
    {tan2=NA}
    ...
    Tan_dependence_Wei_S1=Tan_dependence_Wei_S
    if (is.na(tan2)==F)
    {
      Ratio=1/(((MI*pi))/((ta2sum)))
  ...
  Ne = Tan.dependence.Diffuse_S.single(a, b, tan, MI,wei,tan2)
