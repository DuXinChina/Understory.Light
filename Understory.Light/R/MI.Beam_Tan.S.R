...
#'
#' @return 加权进界邻体直射荫蔽度最适的校正系数
#' @export
#'
#' @examples
#' b=Understory.light::b
#' Understory.light::MI.Beam_Tan.S(minx=50, maxx=100, miny=50, maxy=100, b, seq=5, maxtan=2, mintan=0.5, mi=1.5)
MI.Beam_Tan.S=function (minx, maxx, miny, maxy, b, seq, maxtan, mintan, mi)
{
  ...
  {
    library(tcltk)
    Tan.dependence.Beam_S.single = function(a, b, maxtan, mintan,
                                            mi, MI) {
      ...
      Nei.tree$angle_o = pi - (asin(sin(Nei.tree$angle)/r *
                                      r_) + Nei.tree$angle)
      ...
      Nei.tree_right = subset(Nei.tree, !Nei.tree$x < a[1,
                                                        1])
    ...
    for (j in 1:nrow(a)) {
      ...
      ], b, maxtan, mintan, mi)$Tan.dependence.Beam_S))
  ...
  nls = nls(Percentile_Rank_of_Tan.dependence.Beam_S ~ (atan(Tan.dependence.Beam_S/(MI) -
                                                               pi) + pi/2)/pi, start = list(MI = 1), algorithm = "port")
  res = summary(nls)
  MI = res$parameters[1, 1]
  Revise_Tan.dependence.Beam_S = (atan(Tan.dependence.Beam_S/(MI) -
                                         pi) + pi/2)/pi
  plot(Percentile_Rank_of_Tan.dependence.Beam_S, Revise_Tan.dependence.Beam_S,
       main = "Q-Q plot", xlim = c(0, 1), ylim = c(0, 1)) +
    abline(0, 1, col = "red4")
  R_square = 1 - sum((Percentile_Rank_of_Tan.dependence.Beam_S -
                        Revise_Tan.dependence.Beam_S)^2)/sum((Percentile_Rank_of_Tan.dependence.Beam_S -
                                                                mean(Percentile_Rank_of_Tan.dependence.Beam_S))^2)
