...
plot.Scale.dependence.Beam_S.single=function(a, b, scale, mi,MI,bandwidth)
{
...
Ne = Scale.dependence.Beam_S.single(a,b,scale,mi,MI,bandwidth)
...
if (n != 0) {
  ...
  max = max(abs(Ne$Nei.tree[, 1] - Ne$a[, 1]), abs(Ne$Nei.tree[, 2] - Ne$a[, 2]),scale)
}
...
p = p + geom_circle(data = Ne$a, aes(x0 = x,y0 = y, r = scale), linetype = 2, color = "black",alpha = 1)
